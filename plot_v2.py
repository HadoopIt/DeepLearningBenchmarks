...
def figure(values, names, xticks, cols,
         vertical_lines = [], save=None):
    """Bar plot of values and names    """
    plt.clf()
    barGroups(np.array(values), names, xticks, cols, vertical_lines)
...
def barGroups(A, names,xticks, colors, vertical_lines):
...
if __name__ == '__main__':
    ...
    namesDictionary = [\
             ...
             , ('matlab{gpu/float/60}' ,'Matlab(GPUmat)','k') 
    ...
    figure(values, labels,  ticks, colors,vertical_lines = [2], save = 'mlp.svg')
    ...
    ticks = [ (0.5, 'GPU'), (3, 'CPU')]
    figure(values, labels, ticks, colors, vertical_lines = [1], save='conv.svg')
